...
class Gamelogic:
    ...
    def update(self, events):
        for actor in self.actors:
            ...
            # find die tiles
            try:
                if self.tilemap.current_map[int(math.ceil(bot))][int(left)] in self.tilemap.player_dies\
                or\
                self.tilemap.current_map[int(math.floor(bot))][int(left)] in self.tilemap.player_dies :  
                    actor.on_deadly_stuff()
                else:
                    actor.off_deadly_stuff()
    ...
    def search(self, start1, a, b, dx, dy):        
        start = int(start1)
