...
class Character(Actor):
    ...
    def on_air(self):
        self.land = False
        self.vy = min(self.vy + GRAVITY*self.gravity_factor, MAX_VY)  
    ...
    def draw(self, xcam, ycam):
        ...
        if not self.land:
            ...
            if self.is_dying:
                sprite = self.dying[anim_index][self.frame]
                self.frame = (self.frame + 1) % len(self.dying[anim_index])                
            else:   
